...
export const makeRules = (extraRules) =>
  getRules(baseRules).concat(getRules(extraRules));

const getRules = (raw) =>
  raw.map(({ scope, ...rule }) => ({ ...rule, scope: scope.join(",") }));

const baseRules = [
  {
    foreground: "var(gray)",
    scope: ["comment", "meta.diff.header", "meta.diff.range", "punctuation"],
  },
  {
    foreground: "var(cyan)",
    scope: [
  ...
  },
  {
    ...
    scope: ["comment.tag.fixme"],
  },
  {
    background: "var(yellow)",
    foreground: "var(black)",
    scope: ["comment.tag.todo"],
  },
  {
    font_style: "italic",
    scope: ["markup.italic"],
